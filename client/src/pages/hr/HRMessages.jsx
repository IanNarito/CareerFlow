...
import { 
  ...
  Settings, Building2, MessageSquare, Send, LogOut, Paperclip, X, File, ArrowLeft
} from 'lucide-react';
  ...
  const syncData = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // UX STRICT RULE: HR ONLY sees Job Seekers in their inbox
        const seekerConversations = apiInbox.filter(c => c.role === 'job_seeker' || c.role === 'seeker');

        setConversations(seekerConversations);
      }
      ...
      if (activeChatId) {
        ...
        if (hRes.ok) {
          ...
          setMessages(Array.isArray(apiHistory) ? apiHistory : []);
        }
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 flex overflow-hidden font-sans text-slate-900">
      ...
      <main className="flex-1 flex bg-white text-slate-900 overflow-hidden h-screen">
        ...
        <div className={`w-full sm:w-80 md:w-96 flex-shrink-0 border-r border-slate-200 flex-col bg-slate-50 ${showChatOnMobile ? 'hidden sm:flex' : 'flex'}`}>
          <div className="p-4 border-b font-extrabold h-16 sm:h-20 flex items-center bg-white shadow-sm z-10 text-xl tracking-tight">Candidate Messages</div>
            ...
            {loading ? (
              <div className="p-10 text-center text-slate-400 text-sm font-bold">Loading...</div>
               ...
               <div className="p-10 text-center flex flex-col items-center gap-3">
                  ...
                  <p className="text-slate-500 text-sm font-medium">No candidates have messaged you yet.</p>
               </div>
            ) : (
              conversations.map((chat) => {
                ...
                return (
                  ...
                  >
                    {activeChatId === chat.id && <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-indigo-600 rounded-r-full"></div>}
                    <div className="w-12 h-12 rounded-2xl bg-slate-100 text-indigo-600 flex items-center justify-center font-black uppercase shrink-0 border border-slate-200">
                      {displayName?.charAt(0)}
                    </div>
                    <div className="min-w-0 flex-1 py-0.5">
                      <div className="flex justify-between items-center mb-0.5">
                         <p className={`font-bold text-sm truncate ${activeChatId === chat.id ? 'text-indigo-900' : 'text-slate-900'}`}>{displayName}</p>
        ...
        {/* CHAT PANEL */}
        <div className={`flex-1 flex flex-col bg-slate-50 min-h-0 ${!showChatOnMobile ? 'hidden sm:flex' : 'flex'}`}>
          {activeChatData ? (
            <>
              {/* CHAT HEADER */}
              <header className="h-16 sm:h-20 px-4 sm:px-6 border-b border-slate-200 flex items-center justify-between font-bold bg-white shadow-sm z-10 shrink-0">
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl border border-slate-200 flex items-center justify-center bg-slate-900 text-white font-black uppercase shadow-sm">
                    {(activeChatData.first_name || activeChatData.name || "A").charAt(0)}
                  </div>
                  <div>
                ...
                {messages.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg.message_id || `msg-${idx}`} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] sm:max-w-[75%] lg:max-w-[65%] flex flex-col shadow-sm ${isMe ? 'items-end' : 'items-start'}`}>
                        <div className={`px-4 sm:px-5 py-3.5 ${isMe ? 'bg-indigo-600 text-white rounded-3xl rounded-br-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-3xl rounded-bl-sm'}`}>
              ...
              {/* CHAT INPUT AREA */}
              <div className="p-3 sm:p-6 bg-white border-t border-slate-200 shrink-0 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                {attachment && (
                  <div className="mb-3 flex items-center justify-between bg-indigo-50 border border-indigo-100 p-2.5 sm:p-3 rounded-xl max-w-4xl mx-auto">
                    ...
                    <button onClick={() => setAttachment(null)} className="p-1.5 text-indigo-400 hover:text-indigo-700 hover:bg-indigo-100 rounded-lg transition-colors"><X size={16}/></button>
                  </div>
                )}

                <form onSubmit={handleSendMessage} className="flex items-end gap-2 sm:gap-3 max-w-4xl mx-auto">
                  <input type="file" className="hidden" ref={fileInputRef} onChange={handleFileChange} />
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-full shrink-0 transition-colors">
                    <Paperclip size={22} />
                  </button>
                  ...
                  <div className="flex-1 bg-slate-50 border border-slate-200 rounded-3xl flex items-center focus-within:ring-2 focus-within:ring-indigo-600 focus-within:bg-white transition-all overflow-hidden shadow-inner">
                    <textarea 
                      rows="1" value={messageInput} onChange={e => setMessageInput(e.target.value)} placeholder="Type a message to the candidate..." 
                      className="w-full bg-transparent px-4 sm:px-5 py-3.5 sm:py-4 outline-none text-slate-900 font-medium resize-none placeholder:text-slate-400"
                      onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(e); } }}
                    />
                  </div>
                  
                  <button type="submit" disabled={!messageInput.trim() && !attachment} className={`p-3.5 sm:p-4 rounded-full transition-all shadow-md shrink-0 flex items-center justify-center ${messageInput.trim() || attachment ? 'bg-indigo-600 text-white hover:bg-indigo-700 hover:shadow-lg active:scale-95' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
