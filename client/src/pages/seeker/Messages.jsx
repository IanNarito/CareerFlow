            ...
            <div className="flex-1 overflow-y-auto pb-20 sm:pb-0 custom-scrollbar">
              {loading ? (
                ...
                conversations.map((chat) => {
                  const displayName = chat.company_name || "Unknown Company";
                  return (
                    <button 
                      key={`chat-${chat.id}`}
                      onClick={() => handleSelectChat(chat.id)}
                      className={`w-full text-left p-4 border-b border-slate-100 transition-colors flex items-start gap-4 active:bg-blue-50 ${String(activeChatId) === String(chat.id) ? 'bg-blue-50/50 relative' : 'hover:bg-slate-100 bg-white'}`}
                    >
                      {String(activeChatId) === String(chat.id) && <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-600 rounded-r-full"></div>}
                      <div className="w-12 h-12 rounded-2xl border border-slate-200 flex items-center justify-center bg-slate-100 text-blue-600 font-black uppercase text-lg shadow-sm shrink-0">
          ...
          <div className={`flex-1 flex flex-col bg-white min-h-0 ${!showChatOnMobile ? 'hidden sm:flex' : 'flex'}`}>
            {activeChat ? (
              <>
                <header className="h-16 sm:h-20 px-4 sm:px-6 border-b border-slate-200 flex items-center justify-between shrink-0 shadow-sm z-20 bg-white">
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl border border-slate-200 flex items-center justify-center bg-slate-900 text-white font-black uppercase shadow-sm">
                      {(activeChat.company_name || "C").charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-extrabold text-slate-900 leading-tight text-base sm:text-lg truncate">
                        {activeChat.company_name || "Company"}
                      </h3>
                      <p className="text-[10px] sm:text-[11px] font-bold text-blue-600 uppercase tracking-widest mt-0.5 truncate flex items-center gap-1">
                        <Building2 size={12}/> HR Representative
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-3 sm:p-5 bg-white border-t border-slate-200 shrink-0 pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                  {attachment && (
                    <div className="mb-3 flex items-center justify-between bg-blue-50 border border-blue-100 p-2.5 sm:p-3 rounded-xl max-w-4xl mx-auto">
                      ...
                      <button onClick={() => setAttachment(null)} className="p-1.5 text-blue-400 hover:text-blue-700 hover:bg-blue-100 rounded-lg"><X size={16}/></button>
                    </div>
                  )}

                  <form onSubmit={handleSendMessage} className="flex items-end gap-2 sm:gap-3 max-w-4xl mx-auto">
                    <input type="file" className="hidden" ref={fileInputRef} onChange={handleFileChange} />
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3 sm:p-3.5 text-slate-400 hover:text-blue-600 bg-slate-50 hover:bg-blue-50 rounded-full shrink-0 transition-colors border border-slate-100">
                      <Paperclip size={22} />
                    </button>
                    <div className="flex-1 bg-slate-50 border border-slate-200 rounded-3xl flex items-center focus-within:ring-2 focus-within:ring-blue-600 focus-within:bg-white transition-all shadow-inner">
                      <textarea 
                        rows="1" value={messageInput} onChange={(e) => setMessageInput(e.target.value)} placeholder="Type a message..."
                    ...
                    <button type="submit" disabled={!messageInput.trim() && !attachment} className={`p-3.5 sm:p-4 rounded-full transition-all shadow-md shrink-0 flex items-center justify-center ${messageInput.trim() || attachment ? 'bg-blue-600 text-white hover:bg-blue-700 active:scale-95' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
...
const BottomNavLink = ({ icon, label, to, active }) => (
